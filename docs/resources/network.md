...
- `dhcpd_gateway_enabled` (Boolean) Overrides the default gateway advertised to this network's DHCP clients (the UI's **Default Gateway** Auto/Manual control). When left in **auto** mode (the default) the controller advertises the network's own interface IP as the gateway via DHCP option 3. Set this to `true` to switch to **manual** mode and hand out the address in `dhcpd_gateway` instead — useful for pointing clients at a custom next hop such as a VPN/subnet-router node (e.g. Tailscale).
...
Only meaningful when this network runs the UniFi DHCP server (`dhcp_enabled = true` and `dhcp_relay_enabled = false`); it has no effect on `wan` or `vlan-only` networks.
